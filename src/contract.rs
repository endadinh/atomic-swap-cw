...
use crate::state::{
    _all_swap_ids, AtomicSwap, _CONTRACT_NAME, _CONTRACT_VERSION, _DEFAULT_LIMIT, _MAX_LIMIT, _SWAPS,
};
...
pub fn instantiate(
    ...
    set_contract_version(deps.storage, _CONTRACT_NAME, _CONTRACT_VERSION)?;
    // No setup;
    Ok(Response::default())
}
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Create(msg) => {
            let set_funds = info.funds.clone();
            execute_create(deps, env, info, msg, Balance::from(set_funds))
        }
...
pub fn execute_create(
    ...
    _SWAPS.update(deps.storage, &msg.id, |existing| match existing {
        None => Ok(swap),
...
pub fn execute_release(
    ...
    let swap = _SWAPS.load(deps.storage, &id)?;
    if swap._is_expired(&env.block) {
        return Err(ContractError::Expired {});
    }
...
fn query_list(
    ...
    let limit = limit.unwrap_or(_DEFAULT_LIMIT).min(_MAX_LIMIT) as usize;
    let start = start_after.as_ref().map(|s| Bound::exclusive(s.as_str()));

    Ok(ListResponse {
        swaps: _all_swap_ids(deps.storage, start, limit)?,
    })
}
...
mod tests {
    ...
    use crate::{
        ...
        msg::{CreateMsg, InstantiateMsg},
    };

    use super::*;

    fn preimage() -> String {
        hex::encode(b"This is a string, 64 bytes long.")
    }
    ...
    fn test_create() {
        ...
        let balance = coins(100, "tokens_A");

        // Cannot create, invalid ids
        let info = mock_info(&sender, &balance);
        for id in &["sh", "this_swap_id_is_really_too_long"] {
        ...
        let res = execute(deps.as_mut(), mock_env(), info, ExecuteMsg::Create(create)).unwrap_err();
        assert_eq!(res, ContractError::EmptyBalance {});
        ...
        assert_eq!(
            res,
            ContractError::ParseError("Invalid character \'h\' at position 0".into())
        );
        ...
        let new_balance = coins(1000, "tokens_B");
    ...
    fn test_release() {
        ...
        let release = ExecuteMsg::Release {
            id: "swap9999".to_string(),
            preimage: preimage(),
        };

        let res = execute(deps.as_mut(), mock_env(), info.clone(), release).unwrap_err();
        assert!(matches!(res, ContractError::Std(StdError::NotFound { .. })));

        // Cannot release, invalid hash
        let release = ExecuteMsg::Release {
            id: "swap1".to_string(),
            preimage: "X1s0aa".to_string(),
        };
        let err = execute(deps.as_mut(), mock_env(), info.clone(), release).unwrap_err();
        assert_eq!(
            err,
            ContractError::ParseError("Invalid character \'X\' at position 0".to_string())
        );
